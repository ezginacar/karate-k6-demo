...
Feature: E2E Case Study - Complete Trello Workflow
  ...
  Scenario: Complete E2E Test - Create Organization, Board, List, Cards, Update and Cleanup
    ...
    * def organization = call read('classpath:features/feeders/Organization.feature@createOrganization') { name: '#(faker.getOrganizationName())' }
    * print "A new organization created with id:  >>>", organization.result.id

    # Step 2: Create Board
    * print '\033[1;36m=== Step 2: Creating Board ===\033[0m'
    * def board = call read('classpath:features/feeders/Board.feature@createBoard') { idOrganization: '#(organization.result.id)', boardName: '#(faker.getBoardName())' }
    * print "A new board created with id:  >>>", board.result.id

    # Step 3: Create List
    * print '\033[1;36m=== Step 3: Create List ===\033[0m'
    * def list = call read('classpath:features/feeders/List.feature@createList') { boardId: '#(board.result.id)', listName: '#(faker.getBoardName())'  }
    * print "A new list created with id:  >>>", list.result.id

    # Step 4: Create First Card
    * print '\033[1;36m=== Step 4: Create First Card ===\033[0m'
    * def card1 = call read('classpath:features/feeders/Card.feature@createCard') { idList: '#(list.result.id)', name: faker.getCardName() }
    ...
    * def card2 = call read('classpath:features/feeders/Card.feature@createCard') { idList: '#(list.result.id)', name: faker.getCardName() }
    ...
    * def updatedCard = call read('classpath:features/feeders/Card.feature@updateCardName') { id: '#(card1.result.id)', name: faker.getCardName() }
